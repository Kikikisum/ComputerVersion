...
import torch
import argparse
from utils import RandomResize, RandomHorizontalFlip, RandomCrop, Compose, ToTensor, Normalize
from FCN import fcn_resnet50
from dataset import VOCSegmentation
from eval import create_lr_scheduler, train_one_epoch, evaluate
...
def create_model(aux, num_classes, pretrain=True):
    ...
    if pretrain:
        weight_root = "C:/Users/86188/.conda/yyy/pythonProject1/fcn/weight"
        weights_dict = torch.load(weight_root+"/resnet50.pth", map_location='cpu')
        if num_classes != 21:
            # 官方提供的预训练权重是21类(包括背景)
...
def parse_args():
    ...
    # 构建默认数据路径
    default_data_path = os.path.join(script_dir, "VOCtrainval_11-May-2012", "VOCdevkit")
    # 设置数据集的位置
